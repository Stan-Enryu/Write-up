...
void create_name(){
    ...
    if (0 <= count_name && count_name<=7){
        printf("Input size: ");
        size = readint();
        if (0 < size && size <= 0x420){
...
void delete_name(){
    ...
    printf("Input index: ");
    idx = readint();
    if ((0<= idx && idx <= 7) && name_list[idx].addr_name != 0 ){
...
void copy_name(){
    ...
    dst = readint();
    if (count_copy == 0){
        if ((0<= src && src <= 7) && name_list[src].addr_name != 0 ){
            if ((0<= dst && dst <= 7) && name_list[dst].addr_name != 0 ){

from pwn import *
import string
import gmpy2

def a(p):
	#b *0x804903e
	#b *0x8049076
	#b *0x8049094
	gdb.attach(p,"""
		b *0x804903e
		b *0x80490eb
		
		c
		""")

rev=lambda x: "".join([x[i]+x[i+1]for i in range(len(x)-2,-1,-2)])
stoi= lambda x: ord(x)
# start 0x8049000
data =rev("4030735159026253")
data+=rev("6a4e0929556a1506")
data+=rev("726d79125b612c76")
data+=rev("6e343f2f1e")
data = [int(data[i]+data[i+1],16) for i in range(0,len(data),2)]
print data

data1=[]

sh=process("./soal")

# a(sh)
p = list('COMPFEST12{'.ljust(27,'a')+'}'+"\x0a")

# temp=[]
# param_2 = 0;
# while param_2 < 0x1d :
# 	param_3 = 0
# 	param_4 = 0
# 	# print "######stop######"
# 	while param_3 < 0x1d :
# 		# print "1 :",uVar5
# 		# print "2 :",param_3
# 		# print param_2 + 1 + (uVar5) * 0x1d
# 		param_4 = (param_4 + (stoi(p[param_3]) * data1[param_2 + param_3 * 0x1d] ))% 0x7f
# 		param_3 +=1

# 	temp.append(param_4)
# 	param_2 += 1;
print p
temp=[]
param_2 = 0;
while param_2 < 0x4 :
	param_3 = 0
	param_4 = 0
	print "######stop######"
	while param_3 < 0x1d :
		# print "1 :",uVar5
		# print "2 :",param_3
		print param_2 + param_3 * 0x1d
		# print param_2
		# print param_3
		# print param_4
		print "data1 :",data1[param_2 + param_3 * 0x1d]
		print "data :",stoi(p[param_3])
		print "hasil :",param_4 + (stoi(p[param_3]) * data1[param_2 + param_3 * 0x1d])
		param_4 = (param_4 + (stoi(p[param_3]) * data1[param_2 + param_3 * 0x1d] ))% 0x7f
		break
		param_3 +=1

	print param_4
	break

	temp.append(param_4)
	param_2 += 1;

print temp
print data1[param_2 + (param_3-1) * 0x1d] 
print gmpy2.invert(8,0x7f)
print gmpy2.invert(55,0x7f)

# sh.sendline("".join(p[:-1]))
# x $ebx+$ecx*1+0x804a100
# x/b $ebp+$ecx*1+0x14
# x/b $ecx+0x804a478
# ebp_4=-1
# ebp_10=0x74
# while(ebp_4+1<0x1d):
# 	ebp_4=ebp_4+1
# 	ebp_8=0xffffffff
# 	ebp_c=0
# 	ebx=ebp_4
# 	while(ebp_8+1<0x1d):
# 		eax=ebp_8 & 0xff
# 		ecx=0x1d
# 		eax=ecx *eax
# 		ecx=data[eax+ecx*1]
# 		ecx=0
# 		ecx=ebp_8


sh.interactive()
 
 